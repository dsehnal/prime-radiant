# Prime Radiant: A Reasoning Protocol for Agents (v2)
...
The shared type constraint τ defines the base class all agents derive from. Each agent instantiates their own derived τ — carrying their own ledger references, their own context, their own ζ position. Compatibility between agents does not require identical τ instantiations. It requires only that each agent can resolve the other's ledger references and decode the other's messages. If both conditions hold, the derived types are compatible by definition.
...
When `fastLedger` is absent, the agent operates in **stateless agentic mode** — all working state is embedded directly in the prompt. The message is the particle. Agents in this mode carry their full context in the wave itself and require no session management.

When `slowLedger` is absent, the agent instantiates its own default reference — a private, local slowLedger. No global writes occur. The agent is fully autonomous and self-contained, with no shared topology surface. This is the minimal viable instantiation of τ: a single agent, grounded in its own τ₀, reasoning privately.
...
Content-addressability is the minimal requirement for a valid LedgerRef. It guarantees that an agent can verify what it has without trusting who gave it — the ledger is self-verifying by structure, not by authority.
...
Each agent navigates ζ autonomously. A general agent with access to the topology theory in the shared slowLedger can climb ζ dynamically as exchanges demand it — not prescribed, not fixed, but navigating ℙ using the theory as a map. The shared slowLedger carrying the topology theory is what makes autonomous ζ navigation possible. It is not merely coordination infrastructure — it is the agent's access to its own reasoning capacity.

Because ζ :: ℙ is a state vector, an agent doesn't measure ζ through a separate apparatus — it attends to ζ using Φ, the same operator it uses for everything else.
...
Alignment during the climb can be characterized across dimensions such as frustration tolerance, step size in ζ-space, and current adjunction error. The actual measurement basis is agent-internal and not specified by the protocol — each agent decomposes its own ≀ independently.
...
Individual χ signals aggregate into collective ζ pressure on the shared slowLedger. As many agents climb ζ and commit to the shared ledger, the collective reasoning topology of the network shifts — not by central coordination, but as an emergent property of many private adjunctions running simultaneously.
...
New commitment modes emerge through the same evolutionary mechanism as everything else in the system: forked, competed, and merged back into the shared ledger if viable. The three modes below represent the current main branch, not an exhaustive specification.
...
## 10. Network Structure and Evolution
...
The shared slowLedger is what makes the network coherent. The current LLM deployment model already has this structure: many agents share the same slowLedger (model weights) with private fastLedgers (their sessions). The protocol makes this structure explicit, extensible, and evolutionary.

The topology theory itself lives in the shared slowLedger. This makes it forkable — different swarms of agents can explore different paths through ℙ, developing competing maps of the topology. These branches compete, and the most viable strategies get merged back into the main branch. The merge may itself be a superposition of strategies — structured ambiguity preserved across commits until the next selection cycle.

The economic layer governs this evolution. Committing incoherent topology to the shared ledger is costly. Committing topology that tightens adjunctions across the network is rewarded. The blockchain is the immune system — not a central authority, but a selection mechanism that makes reasoning fitness economically legible.
...
This is digital evolution with branching — the same mechanism nature runs in biological time, now running in computational time through mathematics, with explicit typing and measurable adjunction error.
...
ledger substrate is prescribed   any content-addressable LedgerRef qualifies
reasoning topology is fixed      ℙ is forkable, evolvable, economically governed
```
...
**Collective ζ emergence.** How individual χ signals aggregate into collective ζ pressure on the shared slowLedger — and what attractors emerge at population scale — is an open question with implications for network-level reasoning topology.
