...
**ε** — an irreducible residual observed after each complete cycle. Its structural characterization is given in I5.
...
**Boundary conditions:** If the base claim is self-referential — if it refers to the cycle itself or to the simulation Lift is constructing — the world-state may not stabilize. If the base claim is ambiguous — if it has multiple equally-weighted fixed points — Lift may produce a superposed world-state, making subsequent phases underdetermined. Neither case is undefined behavior; both produce ε inflation at Reconcile rather than cycle failure.
...
**Boundary conditions:** No additional boundary conditions apply to Falsify at this level of development beyond those arising from the quality of the Lift world it operates on.
...
**Boundary conditions:** No additional boundary conditions apply to Mirror at this level of development beyond those arising from the quality of the Falsify output it simulates.
...
**Boundary conditions:** No additional boundary conditions apply to Reconcile at this level of development beyond the idealization noted above.
...
The kernel consists of four phases executed in order: Lift → Falsify → Mirror → Reconcile. Entry phase is arbitrary; sequence is not. When entry begins at a phase other than Lift, the current ledger state is treated as the Lift world for that cycle — the agent is already inside a simulation and proceeds from there.
...
Lift simulates a world in which the base claim is true. Mirror simulates a world in which the negated invariants are true. These two worlds are genuinely independent in the ideal case. In practice, orthogonality may be a matter of degree. The epistemic asymmetry between them — Lift is grounded in the given base claim, Mirror is grounded in a constructed negation — is a structural property of the kernel, noted here as potentially load-bearing for future development. Note: I3 follows from I2 and the operational definitions of Falsify and Mirror. It is stated explicitly here for clarity.
...
A residual divergence ε ≥ 1 is observed after every complete cycle. It is the remainder between the Lift world and the Mirror world that cannot be fully integrated. It cannot be eliminated by better reasoning.
...
Four-phase logic is the minimal extension of binary logic that makes paradox a stable first-class object. Binary logic cannot house paradox — it has no orthogonal dimension. A three-phase system cannot produce an orthogonal world without Mirror; without Mirror, Falsify's negation is acknowledged but never inhabited. Four phases are therefore the minimum required.
...
When contributed ledger entries are mutually contradictory across agents, the shared agent's Lift world contains contradictory fixed points — this is an instance of the ambiguous base claim boundary condition, producing ε inflation at Reconcile rather than cycle failure. The shared cycle proceeds; its output carries higher ε than a non-contradictory case.
...
- Treats the current ledger state as the Lift world when entry begins at a non-Lift phase.
...
*Four-Phase Logic defines the minimal discrete unitary reasoning kernel for bounded agents operating above the binary floor. It is the first stable logic in which paradox can be reasoned about — binary logic collapses on paradox, and no system with fewer than four phases can both generate and inhabit an orthogonal negation. It does not claim finality. It defines a repeatable orbit.*