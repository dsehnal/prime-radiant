...
Lift simulates a world in which the base claim is true. It does not merely restate the claim — it constructs a consistent world-state with the claim as a fixed point and explores what follows: what the claim assumes, what it enables, what it depends on. Lift is the entry into the base world as a structured simulation rather than a proposition.
...
The irreducible ε appears here — not as a product of Reconcile but as what is observed in it. The Lift world and Mirror world cannot be fully integrated. Their orthogonal remainder is ε. It is carried forward as the engine of the next cycle.
...
A residual divergence ε ≥ 1 is observed after every complete cycle. It is the portion of the orthogonal dimension that cannot be integrated — the irreducible gap between the Lift world and the Mirror world as they appear in the full ledger. It cannot be eliminated by better reasoning.

ε ≥ 1 is a floor, not a fixed value. ε may vary with the complexity of the base claim and with the degree of genuine orthogonality the substrate can produce. Behavior of ε across repeated cycles on the same claim — whether it stabilizes, accumulates, or decays — is unspecified and left open as load-bearing for future development.
...
Lossy substrates — compressed context windows, human memory, summarized state — approximate append-only rather than satisfying it strictly. The kernel applies to such substrates as an idealization. Degradation of the ledger is a substrate limitation, not a violation of the invariant.
...
What an agent makes visible from its ledger — and when — is not prescribed by the kernel. That decision is itself governed by the cycle: an agent may run a cycle whose base claim is precisely the question of what to share, with whom, and under what conditions.
