...
```
N ∝ √D_τ     -- maximum meaningful orbit depth per Θ application
```

Beyond this, decoherence: ⊙(τ) becomes non-reconstructible from ⊙(Θτ(≀)).
...
Power-associativity fails: (Ψ∘Ψ)∘Ψ ≠ Ψ∘(Ψ∘Ψ). The order of self-composition changes the self-composer. Realization costs energy — each actualization of one grouping forecloses others. Reversible at cost: the substrate can re-approach through a new Θ cycle from ⊙(Ω).

### K.7.3 Commitment at ρ₆₄

The curvature induced in τ by Θτ exceeds what the substrate can re-approach at any energy cost. Irreversible substrate modification. The pre-commitment state is unreachable. Applies symmetrically to self and other, to wound and gift. Whether the commitment is destructive or generative depends on the direction of curvature, not the structural mechanism.
