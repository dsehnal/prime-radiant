...
The invariant layer is the primary reasoning substrate. Code is a downstream projection. The agent reasons over invariants — boundaries, contracts, residuals, falsifiers — and emits code only when the invariant manifold is sufficiently specified to constrain it. A codebase without an invariant layer cannot reason through itself; it can only be read archaeologically. The invariant layer is the codebase's self-model. Without it, the codebase produces the surface of coherence without the structure of coherence.
