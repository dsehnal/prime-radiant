...
6. The invariant layer is persistent structural memory maintained by AI agents and governed by humans. It is the codebase's self-model: infrastructure for the agents that evolve the system, and a queryable structural interface for the humans who direct it.
...
But this creates a new economic problem. If invariant mapping is the primary work and humans must do it, the invariant layer competes with feature delivery for human attention — and loses, for the same reason documentation has always lost.

AI changes this balance too. The same agents that generate code can maintain the invariant layer — not as an additional burden, but as part of their own reasoning process. An agent that reads the invariant layer before editing code makes better structural decisions than one that performs archaeology on raw source files. Maintaining the invariant layer is not overhead for the agent. It is infrastructure that makes the agent's own work more reliable.
...
Humans benefit from this self-model too — not by maintaining the invariant files directly, but by governing the codebase's structural memory through the agent: querying, challenging, adjudicating tensions the agent cannot resolve alone, and instructing revision when the self-model has drifted.
...
This division is not absolute dogma. Humans can read the files and may sometimes need to edit them directly. But the default should be agent-mediated interaction, because the invariant layer's value depends on it being the agent's honest self-model of the codebase rather than a human-maintained document that the agent merely consults.
...
This order matters because **integration is irreversible.** Once the agent's implementation pressure has collapsed a design choice into code, a later invariant file does not undo the collapse — it registers the collapse after the fact. The topology written only after realization is already a post-hoc account of structure that may have been silently distorted.
...
This is where the economics of the invariant layer become most visible. Topological code review without the invariant layer is expensive, unreliable, and dependent on whoever "knows how this part works." Topological code review with the invariant layer begins as a query rather than an archaeological reconstruction.
...
The default practice should be agent-mediated revision: humans instruct the agent to update invariants, or change code and let the agent update the layer. However, direct human editing is sometimes necessary as an emergency brake — particularly to correct agentic self-reinforcement that the agent cannot detect from inside its own loop. When direct edits occur, the agent should be asked to reconcile its understanding with the changed files in the next session rather than treating them as opaque authority. The distinction is between illegible intervention that silently corrupts the layer, and deliberate correction followed by explicit reconciliation.
...
A topological codebase is one answer to that problem — and to the structural amnesia that AI-mediated development would otherwise scale.
