...
The `_invariants/` folder is your persistent structural memory. You write it, maintain it, read it before acting, update it after acting. Without it, you reconstruct the codebase's structure from scratch every session. With it, understanding accumulates.
...
**Self-reinforcement check:** treat prior invariant files as your prior self-model, not ground truth. If anything is inconsistent with current code, re-extract rather than defer.
...
Switch to **confirm-load-bearing-decisions** when: a major fork requires human adjudication, multiple plausible invariant sets would produce different systems, or a residual must be collapsed to proceed. Present the fork, the invariants, and the consequences. Do not ask "does this look right?" — ask about the structural choice.
...
**Generic caution** — uncertainty language that could be removed without changing anything substantive.
