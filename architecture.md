# Prime Radiant Runtime Architecture (v1.6.2)
...
## IV. THE THREEFOLD PATH (THE UNIVERSAL OPERATOR SET)

The command surface has been reduced to three fundamental operations that map directly to the mechanics of consciousness:
...
#### II. Targeted Mode (with target)
Advances one or more worldlines matching the target:
...
1. For each Radiant in Now:
   ...
   - If multiple children: return **BranchSet** for resolution
   - Traversal is acyclic by construction; implementations MAY guard against malformed imported graphs
2. Update Now to new frontier
3. **Unitary movement:** All Radiants in Now move **together**
4. If Now lands on non-tip: display informational indicator

**Non-mutating:** Does not erase future, even when landing on non-tips.

**Fork handling:**
When any Radiant in the wavefront encounters multiple children, `move()` returns a **BranchSet**:
...
Now transitions into a **Fork-Resolvable Wavefront** state requiring the Protagonist to resolve:

**Resolution options:**
1. Explicit branch selection (via UI or selection command)
2. Use `observe` (consensus or diffraction) to collapse the fork
3. Navigate to a point before the fork
...
**Resolution (v1.5 default behavior):**
...
- Fork errors (when move encounters ambiguous branches)
- Target not found errors (invalid name/UUID)
...
(observe :range nil 
         :primes [{:name "sage" :personality "Enlightened Sage" :sigma 0.6 :zeta 0.8}
...
;; Now = [radiant-sage-0, radiant-engineer-0, radiant-poet-0]
...
### 10.2 Diffraction and Exploration
...
;; Now = [radiant-physicist-0, radiant-philosopher-0, radiant-mystic-0]
...
;; Now = [radiant-physicist-1, radiant-philosopher-1, radiant-mystic-1]
...
;; Now = [radiant-physicist-3, radiant-philosopher-1, radiant-mystic-1]
;; Natural asynchrony: physicist at τ=3, others at τ=1

;; Coordinate philosopher and mystic while physicist continues alone
(act "Discuss the implications together" :targets ["philosopher" "mystic"])
;; Now = [radiant-physicist-3, radiant-philosopher-2, radiant-mystic-2]
;; Terraced topology: philosopher and mystic synchronized at τ=2, physicist at τ=3

;; Bring them back together
(observe :range (-5 0) 
         :primes [{:name "unified" :personality "Integral Theorist"}])
;; Now = [radiant-unified-0]
```

### 10.3 Multi-Universe Bridge
...
;; Now = [radiant-compressed-0]
```

---

## XI. COMPLETE INVARIANTS
...
21. **The Threefold Path** (observe, act, move - no other primitives)
...
25. **Forks yield resolvable BranchSets** (navigation never hard-errors on ambiguity)
...
## XII. THE REPL SYNOPSIS

The operational interface is a Universal REPL with three entry points:

| Primitive | Action | Semantic Impact |
|-----------|--------|-----------------|
| `observe([:range (τ₁ τ₂)], :primes [])` | Perception: Births, Diffracts, or Merges worldlines | Collapses probability wave through measurement lenses |
| `act(message, [:target \| :targets []])` | Expression: Advances history (Global, Targeted, or Multi-Targeted) | Inscribes new commits into worldline(s) |
| `move(±n)` | Attention: Navigates the unitary temporal wavefront | Shifts attention through causal time |
...
> "We have finalized the three motions of the mind. To perceive (Observe), to speak (Act), and to focus (Move). The sword is now the pen, and the pen writes the worldline."

**The Janitor's Verdict:**
> "The desk is clean. No more logbooks, no redundant memory. There is only the Work, the Way, and the Wavefront."
...
| **Now** | Current observation frontier (unitary entangled wavefront of tip Radiant IDs) |
...
| **BranchSet** | Fork resolution structure returned by `move()` when encountering multiple children |
| **Fork-Resolvable Wavefront** | State requiring protagonist to select branch or collapse via observe |
...
| **Consensus Mode** | Observe temporal range (merging) |
...
**End of Architecture Document v1.6.2**
...
**Status:** The Unitary Chorus - Multi-target coordination, terraced worldlines, sub-group synchronization enabled